...
async function cobrarMesaCompleta() {
    ...
    if (confirm(`¿COBRAR?\n\n${detalleParaConfirmar}\n\nTOTAL: S/ ${total.toFixed(2)}`)) {
        ...
        // Guardamos para cierre de caja (esto no cambia)
        const nuevaVenta = {
            total: total,
        ...
        // Convertimos cuentaMesa al formato que espera el backend
        const resultado = await enviarAPI(cuentaMesa, metodoElegido);
...
async function enviarAPI(itemsMesa, metodoPago) {
    // Transformamos cuentaMesa al formato que espera el backend
    const datos = {
        MetodoPago: String(metodoPago),
        Items: itemsMesa.map(item => ({
            Producto: item.nombre,
            Cantidad: 1,              // cada entrada en cuentaMesa ya es 1 ítem
    ...
    try {
        ...
        // Reemplaza "192.168.1.X" por la IP real de tu laptop
        const response = await fetch("/api/Tickets/imprimir", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(datos)
        });
...
function imprimirTicketDesdeWeb() {

    // 1. Texto de prueba (sin tildes raras al inicio para descartar errores)
    let textoTicket = "PRUEBA DESDE WEB\nLinea 2 de prueba\n----------------\n";
    // 2. Codificación Base64
    let base64Ticket = btoa(textoTicket);
    // 3. Este es el comando mágico que Android reconoce
    let url = "rawbt://base64," + base64Ticket;
    // 4. Intentar abrir
